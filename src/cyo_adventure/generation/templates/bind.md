...
Respond with valid JSON only: a single flat JSON object mapping every slot id
listed above to its bound string value, for example
`{"HERO": "Priya", "A1_GATE": "the jammed pressure hatch"}`. Do not include
markdown fences, prose, or any other content before or after the JSON. Do
not omit any slot id listed above, and do not add any slot id that is not
listed above. The validator will parse your response as JSON; any non-JSON
content will cause this step to fail.
