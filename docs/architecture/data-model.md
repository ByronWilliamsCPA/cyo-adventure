...
erDiagram
    ...
    generation_job {
        ...
        int provider_call_count "NULL = not recorded, never 0"
        int provider_unknown_calls "NULL; calls the backend gave no counts for"
        int input_tokens "NULL"
        int output_tokens "NULL"
        int provider_duration_ms "NULL"
        numeric(12,6) cost_usd "NULL; lower bound unless cost_complete"
        boolean cost_complete "NULL; false = a model had no price entry"
        timestamptz created_at
        timestamptz updated_at
    }
