...
| Diagram | Files | Primary sources | Description |
| ------- | ----- | --------------- | ----------- |
| Generation Pipeline | [component-generation.puml](component-generation.puml) / [.svg](component-generation.svg) | `generation/orchestrator.py`, `prompts.py`, `pii.py`, `guarded.py`, `provider.py`, `allowlist.py`, `skeleton_catalog.py`, `binding.py`, `fidelity_gate.py`, `fidelity.py` | Orchestrator, prompts, providers, PII guard, WS-2 theme binding, Stage-1 fidelity gate |
| Validator Gate | [component-validator.puml](component-validator.puml) / [.svg](component-validator.svg) | `validator/gate.py`, `layer1.py`, `policy.py`, `band_profile.py`, `layer2.py`, `reading_level.py`, `safety.py`, `choice_grammar.py` | L1 / Policy / L2 / RL / CG / SAFE layers; also notes sentinel_integrity.py, slots.py, theme_leak.py, series.py as siblings not called by run_gate() |
...
| Publishing | [component-publishing.puml](component-publishing.puml) / [.svg](component-publishing.svg) | `publishing/state_machine.py`, `service.py`, `catalog_publish.py`, `api/approval.py`, `review_surface.py`, `events/writer.py` | Approval state machine, admin-only approve (ADR-005), standalone catalog-publish CLI |
