...
| Sitemap and Flows | [sitemap-and-flows.puml](sitemap-and-flows.puml) / [.svg](sitemap-and-flows.svg) | `frontend/src/router.tsx`, `routes.ts`, `routeElements.tsx`, `landing/LandingPage.tsx`, `auth/DeviceAuthorizedRoute.tsx`, `AdultGate.tsx`, `ProtectedRoute.tsx`, `GuardianVerificationPage.tsx`, `legal/PrivacyPolicyPage.tsx`, `legal/SupportPage.tsx`, `api/kws_redirect.py` | All 34 mapped surfaces: every React route, the two auth-boundary crossings (ADR-014), the four `ProtectedRoute` interstitials in the order it checks them, the `*` catch-all and the `RouteError` boundary, and the full public zone. The public zone is **not** derivable from `router.tsx` alone: `GET /api/v1/consent/kws/return` is a server-rendered HTML page outside the SPA |

## User journeys

| Diagram | Files | Primary sources | Description |
| ------- | ----- | --------------- | ----------- |
| End-to-End Journey | [journey-end-to-end.puml](journey-end-to-end.puml) / [.svg](journey-end-to-end.svg) | `frontend/src/router.tsx`, `landing/LandingPage.tsx`, `auth/DeviceAuthorizedRoute.tsx`, `AdultGate.tsx`, `library/RequestStory.tsx`, `guardian/RequestsPage.tsx`, `reader/ReaderPage.tsx` | Target-state UX across Child/Guardian/Admin/System lanes. Steady-state only: the ADR-018 D1 first-run interstitials (verify, awaiting-approval, consent) live in [journey-guardian](journey-guardian.puml), and the full page map in [sitemap-and-flows](sitemap-and-flows.puml) |
...
| Journey Test Coverage | [journey-dev-coverage.puml](journey-dev-coverage.puml) / [.svg](journey-dev-coverage.svg) | `frontend/e2e/*.spec.ts` (Playwright), `frontend/src/**/*.test.tsx` (Vitest) | Journey recolored by e2e / unit / none coverage. **The colours are a 2026-08-04 snapshot**; the spec inventory was refreshed 2026-08-14 against the 45 files then on disk, which surfaced ten specs the inventory had never picked up (listed in the header, unmapped to nodes). No tool checks either date. This diagram bakes a claim into a committed artifact, so a stale green reads as reassurance rather than as an omission. Per-element uncertainty belongs in a `<<unverified>>` stereotype ([STYLE_GUIDE.md](STYLE_GUIDE.md#status-stereotypes)), which an audit can actually find. |
