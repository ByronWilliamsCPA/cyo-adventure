...
| Google Perspective | **Live classifier leg retired 2026-08-26** (PR #764, issue #659): it no longer receives generated prose or child-typed request text. The only residual receipt is catalog prose from the offline calibration script `scripts/capture_stage0_baseline.py`, which sets `doNotStore: true` | Kept on this table under the removal rule below, not removed: `PERSPECTIVE_API_KEY` (inert, `core/config.py`) and `PERSPECTIVE_URL` (`moderation/classifiers.py`) still reference it, so it is not fully decommissioned. DPA/SCC execution (Phase 5a) stays open for prose and child text disclosed before 2026-08-26; PII egress guard applied as of #304 |
