...
**This table governs.** The first seven rows reproduce, without modification, the
...
In practice, the company's two guardian-facing deletion routes, `DELETE /api/v1/profiles/{id}`
...
- On-demand account/profile deletion: `DELETE /api/v1/profiles/{id}` and
