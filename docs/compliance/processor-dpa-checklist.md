...
| Google Vertex (OpenRouter sub-processor) | Story-generation prompts (PII-guarded), for Google-family traffic OpenRouter routes here since the 2026-07-28 ZDR toggle disabled AI Studio endpoints | As above: via OpenRouter's sub-processor terms | **Sub-processor, not a direct counterparty**; same mechanism as the Bedrock row. Distinct from the Google Gemini row below, which is a direct integration | Not applicable as a direct DPA; same open item as the Bedrock row |
...
| Google Gemini (cover art) | Cover-art prompts | [cloud.google.com/terms/data-processing-addendum](https://cloud.google.com/terms/data-processing-addendum) (the Cloud DPA) | Confirm the Cloud DPA covers the Gemini API tier this account uses before treating this row as closed. **Perspective removed from this row 2026-09-05 (issue #659)**: PR #764 (`b2273a7`, merged 2026-08-26) retired the Perspective leg, so its unresolved Cloud-DPA coverage question no longer needs an answer prospectively. It still bears on prose and child-typed text sent before that date | Not yet executed |
