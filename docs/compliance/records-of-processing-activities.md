...
Google Gemini, Cloudflare R2, Sentry, Epic Games (Kids Web Services).

**Removed 2026-09-05 (issue #659)**: Google Perspective was on this list as a Stage-0 classifier
recipient until PR #764 (`b2273a7`, merged 2026-08-26) retired that leg. It receives no personal
data on any live path now; the only code that still calls it is the offline calibration script
`scripts/capture_stage0_baseline.py`, which sends catalog prose (not personal data) and sets
`doNotStore: true`. Records of the pre-2026-08-26 disclosure remain in scope for any data-subject
request covering that period.
