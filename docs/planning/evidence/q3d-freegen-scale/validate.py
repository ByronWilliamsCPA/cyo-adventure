...
def check(path):
    ...
    if not (100<=len(nodes)<=115): f.append(f"C1 node count {len(nodes)}")
    ...
    if sp<12: f.append(f"C9 shortest positive path {sp}")
    ...
    dfs(start,1,{start})
    if best>24: f.append(f"C10 longest path {best}")
    # C11 ending schema
    es=[n["ending"] for n in nodes if n.get("is_ending") and n.get("ending")]
    if not (14<=len(es)<=20): f.append(f"C11 ending count {len(es)}")
    ...
    if len(ks)<4: f.append(f"C11 only {len(ks)} kinds")
    return f,len(nodes)

for p in sorted(glob.glob("/tmp/claude-0/-home-user-cyo-adventure/8209cdae-3734-5b89-b6c4-9f834b3614c2/scratchpad/redo/freegen/s*.json")):
    try:
        f,n=check(p)
