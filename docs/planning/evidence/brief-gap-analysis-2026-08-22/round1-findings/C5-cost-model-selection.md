...
`skeleton-sourcing-test-plan-2026-08-21.md`, register rows `S-0`..`S-5`, `AL-509`..`AL-513` and
...
- **Problem**: Direct answer to the audit's enumeration question. Brief 4.5 names three measured waste
  ...
  | --- | --- | --- |
  | Non-converging repair | `max_repairs=3` plus the `_signature` no-progress abort (`orchestrator.py:321,879`) | Yes, bounded and logged. Best-covered mode. `AL-512` measured the unbounded version: 14 of 15 blind grid points censored at the cap |
