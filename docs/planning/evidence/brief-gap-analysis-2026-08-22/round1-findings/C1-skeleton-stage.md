...
`S-0`..`S-5`, and `AL-509`..`AL-513` as missing. That was an artifact of the branch I was given.
...
- **Problem**: `admissible_topologies` is a three-way switch. Any acyclic graph with **one**
  ...
  no design read as a tree". `AL-513` records the same. In the blind condition PL-18 is the joint-most
...
- **Problem**: `--strict` escalates six advisory rules to blocking by matching `finding.rule_id` against
  ...
  condition (6 of 15 failures) despite being pure arithmetic with the answer printed. `AL-511` records
...
- **Problem**: Across the 42 tool-assisted grid points: passes take a **median of 5** checker
  ...
  received at each step or what it changed in response. `AL-512` records the corresponding blind-condition
