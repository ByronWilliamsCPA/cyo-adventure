...
> **Census note, added 2026-08-30. The 86 / 15,507 figures in this evidence set are a glob defect,
> not a second valid census.** Both numbers come from listing `skeletons/**/*.json` and excluding
> only `.contract.json` and `.lineage.json`. That leaves the **two** `.narrative.json` sidecars in
> the population, which is where 86 minus 84 and 15,507 minus 15,470 both come from. Reproduce it on
> `origin/main`: the tree holds **149** JSON files under `skeletons/`; excluding contract and lineage
> leaves **86**; also excluding narrative leaves **84**. The authoritative count is
> [catalog-census.md](../../../catalog-census.md), generated by `scripts/catalog_census.py`, and it
> reports **84** shells and **15,470** nodes, with **81** shells declaring
> `metadata.production_eligible` and **74** of those reachable through one of the **18** cells
> `offered_cells()` offers. Those last two are different quantities and neither is "the catalog
> size". Use 84/15,470 as the denominator everywhere in this evidence set; where a derived
> percentage below was computed against 86, it is roughly 2.4% low.
>
> This finding's own 84 / 15,470 is the correct pair. `B2-pipeline-architecture.md` and
> `B3-evidence-methodology.md` report 15,507 and (in B3) 86 nodes-bearing files; those are the
> defective glob, and each now carries the same note.
