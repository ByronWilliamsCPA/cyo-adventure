...
six rater verdicts, and the `S-`, `AL-509..513` and `UW-C317..320` rows. **B1-7 as originally written is
...
`authoring-lessons-log.md` (through `AL-513`), `deepseek-v4-pro-live-fill-plan-2026-08-20.md`,
...
7. **The upstream records are markedly more careful than the brief.** The register's `S-1` status column,
   the evidence README, and `AL-509..AL-513` disclose the credit halt, the slate revision, the data contact
...
- **Problem**: Reading the real run data resolves one of my first-issue objections and replaces it with two
  ...
  *yield*.
  (b) **The blind arm is not a regime anyone uses, and the programme knows it.** `AL-512` states it plainly:
...
- **Recommendation**: (1) Restate F3 as a **yield/convergence** lever. (2) Add one clause: "the blind arm is
  a harness condition, not production practice (`AL-512`); this measures what the checker-in-loop regime is
...
- **How to check I'm right**: `python3 -c "import json;print(json.load(open('docs/planning/evidence/skeleton-author-vendors/runs/e1r3-2026-08-21/run.json'))['cells'])"`
  on tree B returns `['A']`; read `AL-512`'s lesson and proposed-change columns in full.
...
- **Verified against**: tree B (`S-0` register row, `evidence/recognition-protocol-pilot/results.md`,
  `AL-510`, plan sections 5 E0 and 7)
...
- **How to check I'm right**: the `S-0` status column in `diversity-test-register.md` section F; plan section
  7, "Instrument limits"; `evidence/recognition-protocol-pilot/results.md`; `AL-510`.
...
- **Problem**: The S-1 evidence is real, well kept and honestly caveated **in the register**. What the brief
  ...
  (e) **The whole basis is unmerged.** The brief, its evidence, the `S` rows, `AL-509..513`, `UW-C317..320`
