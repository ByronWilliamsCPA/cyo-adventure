...
The concession is fuller in the source than in either the brief or the gap analysis. `AL-510`
...
(`AL-509`), and the substitute was picked without anyone checking where it sat in the catalog's own
...
pre-registration named; what was lost with the deleted branch (`AL-509`) was the W16 *rooftop-
...
1. **The pre-registered cross-band control was never lost.**
   ...
   filled, committed — is the exact skeleton the original pre-registration named. `AL-509`,
...
5. **The recognition instrument has never returned "different" — not once, in any run, manual or
   ...
   the brief, `results.md`, `AL-510`, nor the prior review states it. It refutes the review's
