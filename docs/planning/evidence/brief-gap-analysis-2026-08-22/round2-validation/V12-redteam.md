...
Recomputed under the scope the programme's own pre-registrations use, the D-7b pair is **2.33 shared
4-grams per 1000**, not 3.2, and the margin against the 3.3 idiom floor is **~0.97**, not the
"collapsed" 0.1 the synthesis reports. Corrected 2026-08-30: this headline read **2.55** with a
margin of ~0.75, which is the *label-inclusive* per-unit figure. `V1-f5-d7b.md` reports both scopes
side by side, body-only per-unit **2.33** and label-inclusive per-unit **2.55**, and the scope the
pre-registrations actually use is body-only, which is why V1 concludes the published 2.3 is exactly
right (`7 / 3001.5 x 1000 = 2.33`). Taking the wrong row understated the margin by 0.22. The
direction of the finding is unchanged and slightly strengthened: the margin against the floor is
wider, not narrower, than this red team originally claimed. The synthesis states the mechanism correctly in one sentence and then
...
| C4-2 / B2-3 | critical | high, and misframed | Verified: `api/approval.py` never reads `has_hard_block`. But `api/node_edit.py:666` carries an explicit `#CRITICAL: security:` comment stating this is deliberate, "ADR-005: the human reviewer is the final gate... a fresh block here is persisted and surfaced on the review surface for a human to weigh (approve() does not itself check has_hard_block either)", with a named regression test. ADR-005 itself says automated moderation "gate[s] entry to review; they do not replace the human step". `review_surface.py:130` has an invariant comment ensuring "a bright-line 0.0 BLOCK is never hidden". So this is an accepted design, annotated in three places, not an unnoticed hole. What survives is narrower and still worth fixing: **no attestation that the approver saw the block, and a dual-role adult can self-approve.** *Correction, 2026-08-30: the attestation half is closed. `publishing/service.py:474` (`approve_requires_override_reason`, gate D2 of the ADR-005 amendment of 2026-08-25) refuses an approval over a block or high-severity finding without a typed `override_reason`, and the RELEASED payload carries `overridden_block_count` / `overridden_high_count`. The `node_edit.py` comment quoted here is itself now stale on `main`: `approve()` does check, through `severe_finding_counts()` rather than through the `has_hard_block` symbol. The dual-role self-approve leg still stands, stamped `guardian` by `Principal.acting_role()` but not refused.* |
...
| **B2-23 (F3's tool-assisted regime: the subagent legs have no documented procedure)** | high | **high** | Corrected 2026-08-30. The original row read "exists as code nowhere" and upgraded to critical on that basis; it is false against the tree under review. `scripts/modal_kimi_leg.py` is tracked on `main` and its `--mode tools` path implements the regime for every API leg: `run_grid_point_tools`, a persistent conversation, the checker's full output fed back verbatim, `check_skeleton.py --strict --allow-mvp`, and a `_TOOLS_CHECKER_CAP = 10` invocation cap. See V11's evidence-tree scope note. What survives is narrower and still worth a high: the **four Anthropic subagent legs are driven by an undocumented external procedure**, so that portion of the lever is a practice rather than a mechanism and cannot be handed to anyone. The upgrade to critical does not survive the correction, because the §4.2 conclusions do not rest on a regime with no implementation. |
