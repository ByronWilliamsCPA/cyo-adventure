...
I copied the live log, took `AL-513` (status `open`, the PL-18 topology-trap lesson), and rewrote its
...
register's only citation of `AL-513` (`UW-C306`, rewritten to a dangling id) and crossed that with
the lesson's status:

| register cites AL-513 | status `open` | status `applied` |
|---|---|---|
| yes | PASS | PASS |
| **no** | **FAIL** (`lesson 'AL-513' status is not applied/rejected/superseded and is not cited by any row in cluster C`) | **PASS** |
...
It does not check that a cited id resolves. In my 2x2 I rewrote the register's `AL-513` citation to
...
| `reg_nocite.md` | the register with `AL-513`'s citation dangled, for the 2x2 |
