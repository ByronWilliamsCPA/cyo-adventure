...
- `docs/planning/authoring-lessons-log.md:592` (`AL-512`) — *"The production authoring mechanism
...
**New finding V11-N1 (medium)**: §3.1 and `AL-512` both attribute skeleton authoring to a skill
whose own contract forbids structural change. The real skeleton-authoring procedure is undocumented
ad-hoc session practice. Fix §3.1 and `AL-512`, or write the run book the brief claims exists.
...
3. **#3 conflates two different fixes and hides a blocking dependency in a subordinate clause.**
   ...
   exists at 3-5 and 5-8**" is not a this-week item. `AL-513`'s PL-18 fix is a *message* improvement
   ...
   into 3a (PL-18 message, this week, cheap, safe, and per `AL-513` the highest-yield validator fix
...
| 3 | #3a | PL-18 finding names the first reconverging node | `AL-513`: three legs lost grid points to an unactionable message; message-only, no policy change |
...
brief and in `AL-512`), **V11-N2** (tools arm's pre-registered endpoint degenerate; no blind cell D
