...
```
D1 hard-block ruling ─────────────► W7 approval guard (shape depends on the ruling)
D2 F5 architecture ruling ────────► W10 settling experiment, and any stratified-plan work
...
W9 register integrity ────────────► D2's voiding step is safer after this
```
...
### D1. Rule on the hard-block publish override
...
eight sites, all inside `moderation/`. No DB constraint exists across 63 migrations. The frontend
...
### D2. Rule on what F5 actually claims, and void the edited register rows
...
of them. **Effort** S. **Dependency** none, and `D2` and `W10` both depend on it.
...
|---|---|---|---|
| UW-x1 | E (security/safety) | `D1`: hard-block publish override is untraced, unversioned, and self-approvable by a dual-role adult. Blocked on the production `summary.hard_block` query and an owner ruling. | decision |
