...
| L1-8 | 1 | **Field-minor floor** (ADR-025 decision 3): a document that uses a field introduced at schema minor N must declare a `schema_version` whose minor is at least N. Enforces the converse of ADR-025's stamping clause, so an under-declared document fails the gate rather than being silently admitted. The field-to-minor registry is `storybook/field_minors.py`. | `L1-8 schema: field '{field}' was introduced at schema minor {minor}, but this document declares '{declared}'; stamp it at least {major}.{minor}` |
