...
which read four of the five primary sources directly**. Full entries with sources and caveats are
...
**Two downgrades, one correction, one omission.** The two downgrades are the load-bearing changes:
*The Interlopers* and *The Cavern of the Morlocks* both moved to a stricter status because in each
...
other permission-required title), which has still not been checked; treat it as fully unverified.
...
had unrestricted egress and read four of the five primary sources directly; those entries now carry
`verification_method: primary_source_fetch`, and `data/if-corpus-licensing.yaml`'s
`checked_method_caveat` records the split so a later edit cannot quietly launder pass-1 entries
into pass-2 ones.
...
`green` there now reads, in the file's own header, as "strong lead, not cleared", so a script or a
later agent reading the YAML alone gets the same meaning this document intends. Before any work's
...
- **No change to the licensing register's `status` fields based on anything short of a
  ...
  2026-08-30 that definition is written into
  [`data/if-corpus-licensing.yaml`](./data/if-corpus-licensing.yaml)'s own header rather than
  living only in this prose. The two files previously disagreed: the YAML defined `green` as
  "public domain in the intended jurisdiction, source and rights documented", which reads as a
  clearance, and the YAML is the file a script or a later agent opens alone.
