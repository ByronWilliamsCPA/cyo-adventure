...
| Finding | Effect |
| --- | --- |
| **A14's fix list is 5 nodes across 4 skeletons.** Two earlier figures in this row's history were both wrong because both came from a static direct-terminal scan: 37 was the unscoped count, then 1 was the band-scoped count. The rule as A14 specifies it follows single-choice corridors over the reachable configuration space, which is deliberately broader. Running the implemented rule: `the-quiet-harbor-protocol` `b_lo_final`, `the-cinder-bazaar` `bg_hi_final` and `bg_lo_final`, `the-serpent-vaults` `c22_gate`, `the-sunken-signal` `n_ascent`. All five inspected and genuine. `the-sunken-signal` was invisible even to the corrected static scan because it is MVP tier and excluded from `_production_candidates`, though `load_skeleton` validates it | The rule still lands blocking at S effort, but the four skeletons are strict-xfail quarantined and their content fixes move to S4. Also refutes plan v2 section 6.2's claim that no skeleton gates its survivable option away: `c22_gate` does exactly that |
| ~~**A14's fix list is 1 node, not 37.**~~ (superseded by the row above) All 37 all-negative-valence decisions sit at **13-16 (4) and 16+ (33)**, zero at 8-11 or 10-13. A14 applies the negative-valence reading only at 8-11/10-13, so there it is preventive with nothing to fix; the `death`/`capture` reading at 13-16/16+ catches exactly **1** (`the-quiet-harbor-protocol`) | A14 drops M to S, lands blocking on day one, and the "fix 37 nodes" work item disappears |
