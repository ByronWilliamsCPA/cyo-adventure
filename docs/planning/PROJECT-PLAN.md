...
**Apple readiness follow-up, same day**: that check is now done, confirming the guess
directionally correct with real nuance - see the per-item audit notes on P7-03, P7-04,
P7-06, P7-12, and P7-13 below for what #311 actually shipped versus what's still open.
Also closed by this pass: [ADR-018](./adr/adr-018-childrens-privacy-compliance.md)'s open
decisions D1 (verifiable parental consent mechanism) and D3 (launch geography) each now
have a working decision recorded and implemented in code (signature-capture consent +
OAuth login for D1; US-only storefront confirmed for D3), though both are explicitly
marked pending formal counsel confirmation rather than "closed." D2 (audience
...
| Plan item | Deliverable | Notes |
|-----------|-------------|-------|
| P7-01 | Apple Developer Program enrollment + EU DSA trader status | Prerequisite for P6-05, TestFlight, and submission; publish trader contact details; **audit 2026-07-20**: enrollment type decided as Individual (no separate legal entity for CYO Adventure); the App Store Seller name will show the owner's personal legal name, not a company name. EU DSA trader status is a separate, still-open sub-item of this row: with the storefront confirmed US-only (D3), DSA trader-verification is not yet triggered, but it must be settled before any EU availability. Individual enrollment does not by itself disqualify Small Business Program (15% commission) participation, but SBP admission is separately gated on annual-proceeds and associated-account criteria and stays a separate post-enrollment application, so eligibility is not automatic |
...
| P7-03 | Privacy policy published + linked in-app | Drafted from `privacy-model.md`; hosted at a stable URL (App Store Connect requires one); **Apple readiness audit 2026-07-20**: `docs/compliance/privacy-notice.md` now exists and is substantive (data table, retention schedule, rights, processors, contact), added by PR #311, but it is explicitly labeled "DRAFT" and not published - it is not hosted at any public URL and is not linked from the frontend (only a code comment references it in `onboardingApi.ts:21`; `GuardianConsentPage.tsx`'s consent screen references "our Privacy Notice" with no actual link). Publishing this as a real guardian-facing route and linking it from the landing page, guardian console, and consent screen remains open, and is a hard App Store Connect submission requirement (a public privacy policy URL), not optional polish |
| P7-04 | In-app account deletion (Guideline 5.1.1(v)) | Full-family erasure driven by the child-linked data classification; removes the guardian identity via the Supabase Auth admin API; calls Apple's Sign in with Apple token-revocation endpoint (our code); async deletion job with audit record; drill test proves zero residual child-linked rows; **audit 2026-07-10**: no FK `ondelete` cascade exists on any child-linked table today (contradicts `privacy-model.md` line 133), so the job must add cascades or enumerate every child table plus the `pipeline_event` `BOOK_ASSIGNED` rows (audit C-03). **Apple readiness audit 2026-07-20**: this is now built - `DELETE /api/v1/me/family` (`api/me.py:265-337`) cascades every family-owned table and is drill-tested (`tests/integration/test_deletion_drill.py`, including `test_delete_my_family_removes_everything` and the non-guardian-rejection case), closing the C-03 cascade gap in practice. Still missing: no code anywhere calls an Apple token-revocation endpoint (`grep -rn "revoke" -i src/cyo_adventure` finds only device-grant and family-connection-consent revocation). Currently moot since native Sign in with Apple hasn't shipped (P6-05), but the revocation call must land in the same change that ships native Apple auth, not be added afterward |
| P7-05 | Guardian data export | All family data in a portable format (GDPR access right) |
| P7-06 | Kid-context SDK and telemetry audit | No third-party ads or trackers; any analytics are first-party and anonymized; documented result feeds the privacy nutrition labels; **audit 2026-07-10**: confirmed no Sentry, analytics, ad, or tracker SDK is wired anywhere; include the public cover bucket (P7-11) and the classifier egress (P7-10) in the label inventory (audit H-02/H-03/M-02). **Apple readiness audit 2026-07-20**: `VITE_ANALYTICS_ID` is still declared (`frontend/.env.example:44-45`, `frontend/src/vite-env.d.ts:7`) with zero usages anywhere in `frontend/src` - a dead placeholder, not a live tracker, but worth removing before the privacy nutrition label is drafted so nobody has to explain an "analytics ID" that does nothing |
...
| P7-12 | Confirm third-party processor terms (public tier) | Confirmed still untouched as of 2026-07-20: no commit references processor DPAs/terms; this is a legal/paperwork task, not code, and remains fully open. **Apple readiness audit, same day, correction**: PR #311 (merged after the above check) added `docs/compliance/processor-dpa-checklist.md`, a substantive per-processor tracking checklist - so the paperwork now has a concrete artifact to close against, not zero groundwork. It explicitly self-labels as "not a compliance record": every processor row still shows "Not yet executed." The open item narrows from "no artifact exists" to "no DPA has actually been signed yet" |
| P7-13 | Information-security program + security-doc correction | 🟡 **Half-closed, confirmed 2026-07-20**: PR #304's SECURITY.md-correction half shipped (the stale "no persistent PII without explicit parental consent" claim and the retired-Authentik description were both fixed). The 16 CFR 312.8 safeguards artifact (designated coordinator, risk assessment, vendor-oversight process) and a breach-notification runbook remain unstarted, per the PR's own follow-up commit message. **Apple readiness audit, same day, correction**: this "remain unstarted" claim is now stale - PR #311 added both `docs/compliance/information-security-program.md` (substantive: names a coordinator, scope, and COPPA 312.8/GDPR Art.32 obligations) and `docs/compliance/breach-notification-runbook.md` (substantive, operational Art.33-34 incident procedure with concrete escalation steps). Both artifacts now exist; what remains open is operationalizing and owner sign-off, not authoring them from scratch |
