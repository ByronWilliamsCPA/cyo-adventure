...
| P9-11 | Public-tier generation provider configuration | **Largely satisfied 2026-08-18 by the Ollama retirement**: there is no `ollama_*` config to leave unset and no dev/family-tier Ollama leg to keep out of the public path, because the adapter is deleted. What remains of this item is the Modal leg's posture: Modal is now cascade leg 3 in any tier that configures it (rather than an offline experiment), so the [ADR-010](./adr/adr-010-modal-review-and-gated-generation.md) promotion gate needs restating for a leg that is already on the production path (ADR-003 amendment, 2026-08-18) |
