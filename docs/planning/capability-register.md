...
> **Status**: Active | **Version**: 1.9 | **Created**: 2026-07-16 | **Updated**: 2026-07-28
...
> v1.9 (2026-07-28): **G10** flipped 🟡->✅: `GET /v1/notifications/stream` (authenticated SSE,
> `api/notifications.py`) closes the push-transport gap the row previously carried, consumed by
> `frontend/src/guardian/notificationsStream.ts` and wired into `NotificationBell.tsx` as a
> fallback-preserving addition to the existing poll. **S9** stays 🟡, narrowed: the push half is
> now shipped too, the one remaining piece is a server-scheduled digest job, which this change
...
|----|------------|------|-------|
| K1 | Read with age-appropriate presentation: legible type, UI complexity, and reading level matched to the child's band | ✅ | Bands/reading levels specced (ADR-011); per-band presentation/UX shipped PR #389 (2026-07-24): `KidShell.tsx` stamps `data-age-band`, `band-tokens.css` maps every `AgeBand` to typography/motion tiers, consumed live by `PassageText.css`/`reader.css`. Residual: `--band-tap-min` token defined but unconsumed by `Button.css` (tracked as a trivial follow-up, not a scope gap) |
...
| K9 | Visual library shelf with covers: what's new, in progress, finished | 🟡 | `LibraryPage.tsx` ships a real "Continue Reading" hero plus a "More to Explore" grid, and `BookCard.tsx` shows "Finished!"/progress-bar/"Not started" states, all tested; the in-progress/finished halves are done, only "what's new" is unbuilt (no `published_at` surfaced on `LibraryItem`, no "New" badge/sort anywhere) |
...
| G6 | Edit or reject a generated story (prose tweaks, veto) with re-review on edit | 🟡 | Edit half: `PATCH /storybooks/{id}/versions/{v}/nodes/{node_id}` (`api/node_edit.py`) already authorizes guardians and re-runs the gate/moderation on edit; only the frontend route (`ReviewDetailPage.tsx`) is admin-only, a small UI-wiring gap. Reject/veto half is a bigger gap than previously stated: `approval.py`'s `_load_admin_story` hard-requires `is_admin` in every check (submit/approve/send-back/archive), guardians have **no API path at all** to reject or veto a generated story, not merely a missing UI. Whether guardians should get a veto, or admin is deliberately the sole safety gate per ADR-005, is an open product/ADR decision, not yet an engineering ticket |
...
| G14 | Standard adult auth; multi-guardian households (two parents, a grandparent) | 🟡 | `User.family_id` already supports multiple guardians per family; `POST /admin/users` (merged, WS-J) lets an admin invite a second guardian into an existing family, live and tested (`test_pending_invite_binds_on_first_login_by_email`). Real remaining gap: admin-mediated only, no guardian-initiated self-service "invite my co-parent" path |
| G15 | Device management: authorize and revoke devices, see which books are downloaded where, storage use | 🟡 | Backend mint/list/revoke exists (`device_grants.py`) but the frontend `deviceGrantApi.ts.list()` call is never invoked anywhere; `ConsolePage.tsx` only manages "this device" via localStorage, not the family's device list. ADR-014 itself names a guardian-visible device list as its lost-device mitigation (never built), so this understates the gap rather than overstates it. Download/storage visibility: no backend `offline/` module exists at all, all download state is client-only |
...
| A16 | Generate and manage AI cover art per storybook version, reviewed on the approval surface before it reaches a child | 🟡 | RULED 2026-07-16, recorded in [ADR-017](./adr/adr-017-ai-cover-art.md); shipped (covers/ module, admin trigger, R2 storage, best-effort with fallback). **H2 security gap (security-hardening-plan-2026-07.md) closed on unmerged branch `fix/cover-moderation-gate` (commit b56b11f)**: `generate_cover` now stops at a new `pending_review` status; `approve_cover()` is the sole admin-only path to `ready`. That fix is necessary but not sufficient for this row: it touches zero frontend files, so once merged there is still no admin surface that renders the cover image or exposes an approve action (`CoverStatusView`'s type union doesn't even include `pending_review` yet). This row stays 🟡 until a review UI exists; the remaining gap is "reviewed on the approval surface," not the auto-publish hole H2 already closes |
...
| S9 | Notification/event delivery infrastructure underlying K12, G10, and admin alerts | 🟡 | Shipped 2026-07-17 in PR #270: `notifications/service.py` projects the append-only `pipeline_event` log into the K12/G10 feeds. Push transport shipped: `GET /v1/notifications/stream` (authenticated SSE via FastAPI's native `StreamingResponse`, deliberately not a WebSocket, which `middleware/security.py` disclaims support for) reuses this same `list_guardian_notifications` query rather than duplicating it, and `NotificationBell.tsx` prefers it over the poll, falling back on connection failure. Narrowed, still-open remainder: a server-scheduled digest job (a periodic batched summary, distinct from both the poll and the SSE push, neither of which is scheduled infra) has not been built and is out of scope for the push-transport work above; no code in this repo implements it |
