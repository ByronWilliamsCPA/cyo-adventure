...
| A5 | Incident path: trace how content reached a child (prompt, model, gate version), pull it everywhere including offline, notify affected guardians | ✅ | Provenance trace and pull-everywhere (offline copies, `frontend/src/offline/revocation.ts`) delivered 2026-07-17. Guardian notification closed on unmerged branch `feat/incident-revocation-notification` (commit 3916e99): `EventType.STORYBOOK_ARCHIVED` (`events/models.py`) now fires from the sole published->archived hop (`publishing/service.py::archive`), and `notifications/registry.py::_compose_storybook_archived` composes it as a G10/S9 alert-severity notice; no frontend change needed, `NotificationBell.tsx` already renders any alert-severity item generically |
