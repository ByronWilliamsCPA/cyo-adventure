...
| A16 | Generate and manage AI cover art per storybook version, reviewed on the approval surface before it reaches a child | 🟡 | RULED 2026-07-16, recorded in [ADR-017](./adr/adr-017-ai-cover-art.md); shipped (covers/ module, admin trigger, R2 storage, best-effort with fallback). **H2 human-approval gate closed 2026-07-28** (security-hardening-plan-2026-07.md): `generate_cover` now stops at `cover_status = "pending_review"` instead of writing `ready` directly; a new admin-only `covers.service.approve_cover` (`POST /storybooks/{id}/versions/{version}/cover/approve`) is the sole path to `ready`, stamping `cover_approved_by`/`cover_approved_at`; the existing `cover_status == "ready"` read filter in `api/library.py`, `api/recommendations.py`, and `api/covers.py` excludes any pending cover with no change needed, and the migration demotes pre-gate `ready` rows with a NULL approver back to `pending_review`. This row stays 🟡, not ✅, for three reasons: an automated image-safety classifier (the image analogue of the text `moderation/` gate) still does not exist, so human approval is the sole layer rather than a second independent one; the gate binds the API read paths, not the stored bytes, whose deterministic R2 key is fetchable without credentials while the bucket is publicly served (verified 2026-07-28, tracked as `UW-M07`); and the backend fix touched zero frontend files, so no admin surface yet renders the cover image or exposes an approve action (`frontend/src/guardian/coverApi.ts` still declares `cover_status` as `'none' | 'generating' | 'ready' | 'failed'`, which excludes the status the backend now returns), leaving the literal "reviewed on the approval surface" clause unmet until PR #471 lands the review UI. The story text guarantee (A6) is unaffected |
