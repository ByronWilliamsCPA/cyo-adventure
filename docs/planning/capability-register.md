...
> G6 edit half (prose editor with gate and moderation re-run, now reachable from both the
> admin surface and a guardian-scoped review/edit route at `/guardian/review/:storybookId`
> for the requesting family's own story; the reject/veto half remains open pending an
> ADR-005 product decision), G7 (complete: consent debits quota on ALL spend paths incl.
