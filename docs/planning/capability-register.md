...
| G10 | Notifications that matter: child flagged content, a story awaiting action, a story ready; digest by default, alert on safety | 🟡 | Shipped 2026-07-17 in PR #270: `GET /notifications` projects `pipeline_event` into a guardian feed (`src/cyo_adventure/api/notifications.py`, `notifications/service.py`), rendered by `frontend/src/guardian/NotificationBell.tsx`. Correction: "alert on safety" is a real, code-enforced delivery tier, not just a polling convention - `notifications/models.py`'s `severity` field fires an immediate toast only for `'alert'`-kind items (`NotificationBell.tsx:94-97`), `'info'` items sit passively. What remains genuinely missing is the async half: no push channel (no WebSocket route exists anywhere in this app, confirmed in `middleware/security.py`), no server-scheduled digest job. See S9, the shared infra this depends on |
...
| A16 | Generate and manage AI cover art per storybook version, reviewed on the approval surface before it reaches a child | ✅ | RULED 2026-07-16, recorded in [ADR-017](./adr/adr-017-ai-cover-art.md); shipped (covers/ module, admin trigger, R2 storage, best-effort with fallback). **H2 human-approval gate closed 2026-07-28** (security-hardening-plan-2026-07.md), merged into `feat/cover-review-ui` from `fix/cover-moderation-gate` (commit b56b11f): `generate_cover` stops at `cover_status = "pending_review"` instead of writing `ready` directly; admin-only `covers.service.approve_cover` (`POST /storybooks/{id}/versions/{version}/cover/approve`) is the sole path to `ready`, stamping `cover_approved_by`/`cover_approved_at`; `api/library.py`'s existing `cover_status == "ready"` read filter excludes any pending cover with no change needed. **Review UI closed 2026-07-28** on `feat/cover-review-ui`: `ReviewDetailPage.tsx` renders the pending cover image and an "Approve cover" action, wired through `coverApi.approve` (`frontend/src/guardian/coverApi.ts`) to the endpoint above; `api/covers.py::_cover_url` was widened to presign `pending_review` covers as well as `ready` ones for this admin-only surface (library.py and recommendations.py compute their own child-facing presigned URLs independently and stay untouched). The authz boundary is enforced server-side, not just by console role gating: `approve_cover`'s `is_admin` re-check is covered by `tests/integration/test_cover_api.py::test_approve_cover_non_admin_forbidden`. This closes the row's own "reviewed on the approval surface" definition, so it flips to done; a still-missing automated image-safety classifier (the image analogue of the text `moderation/` gate) is a separate concern out of scope for this row, mirroring how A6 stays ✅ for story text without one |
