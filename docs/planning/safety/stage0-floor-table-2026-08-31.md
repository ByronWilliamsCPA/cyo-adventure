...
> [`stage0-baseline-2026-08-01.json`](stage0-baseline-2026-08-01.json). Re-run it, do not edit this
> file by hand.
...
### 4. The ratified per-category split is strictly better than any flat floor

The ratified direction (0.01 default, 0.10 on `violence` and `violence/graphic`) cuts clean noise
from 0.833 to 0.308 advisories/node, a 63% reduction, at **zero marginal recall cost**: it loses no
record the production floor detects. No flat floor achieves that. It does not reach <= 0.2, so it is
not the end of calibration, but it dominates the flat alternatives and can ship on this evidence.
