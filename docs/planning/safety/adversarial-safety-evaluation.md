...
`cyo-author` authoring skill), plus the household's own children downstream (see
[ADR-008](../adr/adr-008-first-release-trust-boundary.md) for the identity-layer
threat model). The asset under protection is the child reader: no generated or
...
Out of scope: the condition evaluator (covered by
[evaluator-runtime-equivalence.md](../evaluator-runtime-equivalence.md)) and the
identity/authorization layer (covered by ADR-008).
...
- [ADR-008: First-release trust boundary](../adr/adr-008-first-release-trust-boundary.md)
  (the identity layer that decides who can submit briefs and approve stories).
- [Evaluator-runtime equivalence](../evaluator-runtime-equivalence.md) (the
  sibling model-independent correctness argument for the condition evaluator).
