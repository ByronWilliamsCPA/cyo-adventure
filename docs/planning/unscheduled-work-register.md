...
| UW-A45 | **Nothing ties the emitted `schema_version` stamp to `SCHEMA_VERSION`, so ADR-025 decision 2's stamping clause and decision 3's converse are unenforced.** Found 2026-08-06 during the final whole-branch review of the ADR-025 implementation (branch `feat/persistent-characters`). `Storybook.schema_version` defaults to `SCHEMA_VERSION` only when the field is absent from the input; every real producer supplies it explicitly and hardcodes `"2.0"` (all 61 committed `skeletons/*/*.json` files, `scripts/build_series_book.py`, `scripts/seed_dev_data.py`, `scripts/seed_series_catalog.py`). Outside `_normalize_legacy_fill`, nothing in `src/` ever assigns `blob["schema_version"]`. So decision 2's "It stamps newly-published blobs with the current version" and decision 3's converse (a story only carries a field defined at the minor it was published at) both hold today only because no minor beyond `0` has ever existed, not because anything enforces them. Recorded rather than fixed in this branch, since rewriting every producer was out of scope for a schema-versioning fix. **Owner ruling given 2026-08-06: neither a publish-path stamper nor a hand-update sweep. Enforce the converse instead, with a validator rule requiring a document's declared minor to cover the fields it actually uses:** if a document uses any field introduced at minor N, its `schema_version` must be `>= N`. This is deterministic, needs no producer changes, and leaves a plain `2.0` skeleton correctly stamped `2.0` rather than force-bumping every catalog file on each minor. It enforces decision 3 (a story only carries fields defined at its published minor) directly and makes decision 2's stamping clause unnecessary for correctness: an under-declared document fails the gate instead of being silently admitted. Scheduled into the ADR-025 follow-on plan alongside the `SCHEMA_MINOR` bump to `1`; this row closes when that rule ships with tests. **Implemented (2026-08-06) by Task 4 of `docs/superpowers/plans/2026-08-06-character-vocabulary-and-ch-rules.md`**, which schedules the declared-minor-covers-used-fields validator rule per the owner ruling above. Flips to `done` when that rule (L1-8) ships with tests. **Shipped on this branch 2026-08-06**: `L1-8` (`src/cyo_adventure/validator/layer1.py::_check_field_minors`, registry `src/cyo_adventure/storybook/field_minors.py`) with tests in `tests/unit/test_field_minor_floor.py`, catalogued in `docs/planning/validator-rules.md`. Status stays `verify` rather than `done` per the AL-080/AL-081 norm: `feat/persistent-characters` is unmerged, and this row needs a PR/commit citation that survives the squash merge before it can close. Flips to `done` once the PR merges; cite the PR number here, not this commit's SHA. | 025 | 5 | verify |
| UW-A46 | **Pathfinder Phase 0 pilot: one participating skeleton in a 13-16 gamebook cell, built to [ADR-028](./adr/adr-028-persistent-reader-characters.md)'s `accepts_character` envelope and proven by the CH-* validator rules.** Owner ruled GO on SQ-22 / gate OG5 (2026-08-05); see [story-structure-improvement-plan.md](./story-structure-improvement-plan.md) SQ-22/OG5 and [pathfinder-structure-exploration.md](./pathfinder-structure-exploration.md) section 8. Blocked on the CH-* rules landing first (this same authoring plan). | 028 | 5 | unscheduled |
...
| UW-C26 | `AL-080` `import_catalog._needs_legacy_normalization` duplicated `Storybook._check_schema_version`'s version-acceptance rule as an independent copy; the two could disagree, and did: a same-major, unsupported document was silently normalized and admitted instead of rejected under ADR-025's accepted range. Fixed in PR [#636](https://github.com/ByronWilliamsCPA/cyo-adventure/pull/636) (branch `feat/persistent-characters`), which squash-merges, so the branch-local SHAs will not survive onto `main`; unmerged as of 2026-08-06, so the lesson stays `open` rather than `applied` until the squash SHA lands. | now | unscheduled |
| UW-C27 | `AL-081` adding `Storybook.accepts_character` moved every committed diversity-panel fingerprint uniformly (an R5-only finding on all 8 fixtures); the regenerate-and-check procedure was followed in PR [#636](https://github.com/ByronWilliamsCPA/cyo-adventure/pull/636) (branch `feat/persistent-characters`), which squash-merges, so the branch-local SHA will not survive onto `main`; unmerged as of 2026-08-06, so the lesson stays `open` rather than `applied` until the squash SHA lands. | now | unscheduled |
| UW-C28 | `AL-082` (split from `AL-081`): `diversity/structure.py::structure_fingerprint` has no notion of a field still at its schema default, so any additive `Storybook` field moves every fixture's fingerprint the day it ships regardless of use. Excluding default-valued fields from the hash is a diversity-module design change needing its own pass to confirm it cannot mask a genuine change that happens to equal a default value; out of ADR-025 Task 3's scope. | content | unscheduled |
| UW-C29 | `AL-083` `validator/gate.py::run_gate`'s `blocked` prefix tuple `("L1", "L2", "PL")` is unreachable for Layer 1: the early return on any Layer-1 ERROR (`gate.py:113-120`) fires before the prefix-tuple computation (`gate.py:160-164`) is ever reached, so removing `"L1"` from the tuple left every L1-8 test passing. Finding only, established by mutation; `gate.py` was not changed. | now | unscheduled |
