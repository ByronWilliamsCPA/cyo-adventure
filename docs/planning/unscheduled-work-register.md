...
| UW-D28 | [#601](https://github.com/ByronWilliamsCPA/cyo-adventure/issues/601) | `supabase/config.toml` is committed but never deployed: `supabase-staging.yml`/`supabase-production.yml` run `supabase db push` (migrations) only, no `config push` step exists anywhere in `.github/workflows/`. Production's real `[auth]` policy (password requirements, rate limits, captcha) is whatever was last set by hand in the dashboard, unreviewable and undocumented, including the only throttle on the unauthenticated `resetPasswordForEmail` path. Previously deferred as design-spec follow-up 3 in `docs/superpowers/specs/2026-07-10-supabase-environments-pipeline-design.md` pending CLI stability; `supabase config push` is confirmed supported at the pinned CLI version 2.109.1, so that reason no longer holds. The prerequisite blocker (production was adopted from an already-live project and its `[auth]` block had never been reconciled against `config.toml`) is **cleared**: both projects' live auth config were read from the Management API and diffed field by field against the file on 2026-08-04, and the diff was reviewed before anything was written. Every divergence ran in the loosening direction, confirming the checked-in file was a local-dev file rather than an aspirational policy. Closed on branch `claude/supabase-config-push-issue-1kbz39`: `config.toml` now carries the agreed policy (password floor 10 per NIST SP 800-63B, email confirmations on, OTP length 8, TOTP MFA on, 60s email throttle) plus `[remotes.staging]`/`[remotes.production]` blocks for the two genuinely per-environment fields, and `config push` runs in both deploy workflows between `link` and `db push`. Staging is deployed and verified against a pre-push snapshot with a delta of exactly the two intended fields. Production is wired but deliberately **not yet dispatched**: it stays behind the `production` GitHub Environment's required-reviewer gate, so the first production rollout is a human action, not a merge side effect. Promotion procedure and the snapshot/restore recovery path: `docs/guides/supabase-environments.md` section 4. Follow-on: UW-D29. | 5 | done |
