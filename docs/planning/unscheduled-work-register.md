...
| UW-A45 | **Nothing ties the emitted `schema_version` stamp to `SCHEMA_VERSION`, so ADR-025 decision 2's stamping clause and decision 3's converse are unenforced.** Found 2026-08-06 during the final whole-branch review of the ADR-025 implementation (branch `feat/persistent-characters`). `Storybook.schema_version` defaults to `SCHEMA_VERSION` only when the field is absent from the input; every real producer supplies it explicitly and hardcodes `"2.0"` (all 61 committed `skeletons/*/*.json` files, `scripts/build_series_book.py`, `scripts/seed_dev_data.py`, `scripts/seed_series_catalog.py`). Outside `_normalize_legacy_fill`, nothing in `src/` ever assigns `blob["schema_version"]`. So decision 2's "It stamps newly-published blobs with the current version" and decision 3's converse (a story only carries a field defined at the minor it was published at) both hold today only because no minor beyond `0` has ever existed, not because anything enforces them. Recorded rather than fixed in this branch, since rewriting every producer was out of scope for a schema-versioning fix. **Owner ruling needed from core-maintainer:** after `SCHEMA_MINOR` is next bumped (to `1`), what stamps a newly authored document `2.1`, given all 61 skeletons hardcode `"2.0"` and nothing in `src/` rewrites the field: does the publish path stamp `SCHEMA_VERSION` going forward, or is each producer updated by hand at the next minor bump? | 025 | 5 | decision |
...
| UW-C26 | `AL-080` `import_catalog._needs_legacy_normalization` duplicated `Storybook._check_schema_version`'s version-acceptance rule as an independent copy; the two could disagree, and did: a same-major, unsupported document was silently normalized and admitted instead of rejected under ADR-025's accepted range. Fixed in commits `42439204`/`0b32ddd6` on branch `feat/persistent-characters`, local and unpushed as of 2026-08-06; no PR exists yet and no SHA is on `main`, so the lesson stays `open` rather than `applied` until it merges. | now | unscheduled |
