...
|----|------|-------|--------|
| UW-M01 | Make the staging backend's device-grant config consistent across every backend process, then re-run `e2e-staging.yml`. **Re-diagnosed 2026-08-02; the original "rebuild the stale `:staging` image from `b29aa6b`" ask is refuted and has been rewritten.** That premise fails two ways: every migration since `b29aa6b` is purely additive (no `DROP`/`RENAME COLUMN`), so a forward-migrated database cannot break older ORM code, and `trigger-image-build.yml` has succeeded on every merge through 2026-08-02. The tier's redness was in fact **two independent faults**. (1) Three deterministic 500s (`/v1/notifications`, `/v1/review-queue`, `/v1/guardian/books`, the only three routes issuing a full-ORM `select(Storybook)`/`select(StorybookVersion)`) caused by the 07-30 migration stall, i.e. the database being BEHIND a current backend, the exact inverse of the original diagnosis. **Fixed** by repair run [30754078625](https://github.com/ByronWilliamsCPA/cyo-adventure/actions/runs/30754078625) and confirmed green in run [30757075720](https://github.com/ByronWilliamsCPA/cyo-adventure/actions/runs/30757075720). (2) `POST /v1/device-grants` returns 201 and the very next `GET /v1/profiles` carrying that grant returns 401, which the kid shell renders as "Ask a grown-up to help". **Open.** All other 401 branches are closed by evidence: claims well-formed (`aud`/`iss`/`exp`/`sub`/`role` and three valid UUID ids), grant row present in staging's database with `revoked_at IS NULL`, and the token does not verify against either repo-known `docker-compose.yml` placeholder. Since mint and verify read the same settings singleton, one process is self-consistent whatever its secret, so this requires **two backend processes with differing configuration**: either a differing `DEVICE_GRANT_SECRET` or a differing `DATABASE_URL` (both collapse to the same anti-oracle 401 by design, `api/deps.py` and `core/device_grant.py`). `docker-compose.prod.yml` runs `replicas: ${REPLICAS:-2}` and Portainer applies `stack.env` at container creation, so a stack updated without recreation leaves containers on stale environment; round-robin then alternates consecutive requests, which is why this fails 100% rather than intermittently. Verify by comparing `sha256sum` of each container's `DEVICE_GRANT_SECRET` and `DATABASE_URL` via `docker inspect`; remedy is to redeploy the stack **with container recreation**. | homelab-infra | unscheduled |
