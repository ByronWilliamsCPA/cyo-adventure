...
| UW-A03 | **Production cutover** (issue #559): retire `postgres`-role traffic. Verified live 2026-08-03, the two environments differ and this row previously flattened them: **staging is already cut over for the API** (`cyo_api` has LOGIN, `rolbypassrls=f`, and holds the live app connections; see PR #560's root-cause, which only reproduced because staging is on `cyo_api`), but its worker is unverified and falls back to the API credential when `WORKER_DATABASE_URL` is unset. **Production is untouched**: `cyo_api`/`cyo_worker` are NOLOGIN with no password and the app connects as `postgres` (BYPASSRLS), so every RLS policy is inert there. The ADR names M4.1 as its review gate; M4.1 carries no ADR-021 content. | 021 | M4.1 | unscheduled |
