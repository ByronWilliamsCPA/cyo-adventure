...
| UW-E17 | ADR-015 `G3`'s **per-profile** spend envelope is not enforced. `story_requests/service.py:149` defines `profile_monthly_spend`, but no caller exists: `can_auto_approve` (same file, ~line 296) reaches past it for the private `_approved_count_since` instead, so auto-approval counts requests rather than measuring spend. The sibling `family_monthly_spend` IS wired (`api/story_requests.py:891`), so the family-level cap works and only the per-profile one is inert, which is why nothing looks broken. Found by the 2026-07-31 vulture calibration; related to UW-E06 (`M7` family cost cap). | 5 | unscheduled |
...
| UW-F22 | Five dead symbols confirmed by the 2026-07-31 vulture calibration, all documented as doing something they do not do. (a) `flywheel/strategy.py:81` `OPEN_PR_PER_CELL` is documented as the per-cell open-PR cap, but `flywheel/cadence.py:193` tests set membership instead of reading it, so raising the cap silently does nothing. (b) `flywheel/strategy.py:105` `TEMPLATE_SET_VERSION` is documented as feeding ledger lineage, but `flywheel/ledger.py` never reads it, so no ledger record carries a template-set version. (c) `core/token_audience.py:42` `TokenAudience.GUARDIAN_OIDC` is unreachable in a registry meant to be closed-world: the distinctness validator hardcodes the other two members. (d) `generation/authoring_metadata.py:44` `SkeletonAuthoringMetadata` is a TypedDict no consumer applies. (e) `scripts/check_quality_gate.py:85` `get_measures` is a dead method on a script no nox session, workflow, or hook invokes. Each is either wire-it-up or delete-it, not a judgment call about style. | 5 | unscheduled |
| UW-F23 | Generate the vulture `--make-whitelist` baseline, but only AFTER UW-F22 and UW-E17 are closed, otherwise the baseline freezes the six true positives as accepted. Regenerate at commit time: whitelist entries are line-anchored and drift on any edit above them. Residual finding count after the 2026-07-31 calibration is 77 (from 346 raw), all adjudicated false-positive or test-only. | CI hygiene | unscheduled |
