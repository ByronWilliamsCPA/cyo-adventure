...
| UW-C27 | `AL-083` `skeletons/5-8/the-tide-pool-rescue.json` gives a 5-8 reader five single-choice nodes (385 words) before the first decision, because its four rescue quests run in a hardcoded order. It is the only skeleton in the library past a PL-25 band ceiling and the only one carrying a PL-20 arc warning, so the two findings are one design fault seen twice. Advisory, not blocking. Owner ruling needed between the minimal node merge (`st_meet` into `st_plan`, clears at node 5, changes nothing else) and making the quest order reader-chosen at `n_meet` (fixes the design, rewrites the transition chain and the topology claim). | 4b | unscheduled |
...
| UW-C29 | `AL-086` 20 of 61 committed skeletons open ON their first decision (`start_node` itself offers >= 2 choices), so they sit under the PL-25 band floor. That is a library-wide authoring convention, not 20 independent defects, and it sits outside the entire JHM 2019 corpus, whose measured pages-to-first-decision range starts at 2. Owner ruling needed: either the convention is deliberate (and the floor should be 1 for every band, retiring the warning) or it is drift (and 20 skeletons need an establishing node). Do not normalise silently in either direction. | 4b | unscheduled |
