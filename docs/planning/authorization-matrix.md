...
| Report/remove an offline download (`PUT`/`DELETE /device-downloads`, G15) | Yes (DELETE only, family-scoped) | Own family profile | Own profile only (PUT); own family (DELETE, profile-agnostic) | Own family only (DELETE only; PUT 403s, no `profile_ids`) | `authorize_profile` on PUT; DELETE is scoped by `ctx.principal.family_id` with no role gate, since it mirrors the client's profile-agnostic eviction (`deleteStorybooksById`) |
| List a family's offline downloads (`GET /device-downloads`, G15) | Yes (own family) | Own family only | No (403) | No (403) | Guardian or admin role required, mirrors `device-grants`'s own list gate; family-scoped, no cross-family override |
