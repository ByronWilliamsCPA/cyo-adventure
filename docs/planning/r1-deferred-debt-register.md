...
| --- | --- | --- | --- |
| G1 | **[Closed]** Child-session scoping: the kid surface previously ran under the guardian's token with no child-scoped session or role separation on the wire (cross-reference: safety-eval Finding 6, the same gap, deliberately left unfixed in R1 per the remediation plan's Task C4 decision record). Closed by [ADR-014](./adr/adr-014-device-authorized-kid-access.md) (2026-07-13, device-authorized kid access), verified end to end on this branch: on the backend, `api/deps.py::require_principal` routes a token by its (unverified, then branch-verified) audience claim -- a `CHILD_SESSION_AUDIENCE` token resolves via `_child_principal` to a distinct `Role.CHILD` `Principal` scoped to exactly one profile (`profile_ids` is the signed singleton), verified with its own HS256 secret/audience wholly separate from the guardian's Supabase JWT (RS256/ES256 via JWKS) and from the device-grant token's own secret (`core/child_session.py`, `api/child_sessions.py::create_child_session`, mintable only by a guardian, admin, or device grant, never by a child itself). On the frontend, `frontend/src/hooks/useApi.ts`'s request interceptor attaches the stored child-session token (`frontend/src/auth/childSession.ts`) instead of the guardian bearer whenever the current route is a kid-token route (`isKidTokenRoute`: `/library/*` and `/read/*`, matching `frontend/src/router.tsx`'s `library/:profileId` and `read/:profileId/:storybookId/:version` paths) and the stored session's `profileId` matches the routed profile; a missing or mismatched session is the sole fallback to the guardian bearer (a fresh deep link or just-expired token), not the steady-state path. Pinned by `frontend/src/hooks/useApi.test.ts` ("request interceptor child-token selection", "does not attach a child token whose profile does not match the routed profile") and `frontend/src/auth/childSession.test.ts`. | R1 architecture (accepted for family-internal web); Finding 6 deferral recorded in the R1 remediation plan's Task C4 (merged, doc archived); closed by ADR-014 (2026-07-13) | Closed; no further action |
...
| GS3 | Google Perspective API sunsets 2026-12-31 with no code-level date gate: past that date every Perspective call fails, the circuit breaker opens (`moderation/classifiers.py:82`), and every book gets a `classifier_coverage_incomplete` FLAG that soft-gates it into repair plus full re-moderation for an infrastructure reason, not a content one. Also unusable today: 18 of 29 live storybook_version rows were moderated by the mock reviewer (`review_provider=mock`) and carry safety reports that are evidence of nothing. | `safety/moderation-review-current-state-2026-07-28.md`, `safety/moderation-review-redesign-2026-07-28.md` | High | Ship the redesign's Stage A (structural-failure collapse) first, then retire the Perspective axis (unset `PERSPECTIVE_API_KEY`, remove the classifier leg) no later than 2026-12-31 per the redesign's Stage C and section 3.2 item 5 |
...
| SL10 | WS-G reader v1 carry-state limits: RESTART on a continued book drops carried state (M1), and a hard refresh before the first save loses the location-state seed. **Half closed:** the RESTART limb is fixed in [#648](https://github.com/ByronWilliamsCPA/cyo-adventure/pull/648) (issue #460); the refresh limb is still open, because `ReadingState` records no continuation provenance and the seed travels only in router `location.state` | WS-G PR #192 body | Low | Persist the continuation seed (for example, to IndexedDB) so a read re-entered in a later session still restarts as a continuation. The preserve-or-drop question is settled: RESTART preserves carried state (#648) |
