...
| U8 | Admin story review (`ReviewDetailPage.tsx`) has no version picker or passage-level diff: `reviewApi.ts`'s `surface()` already accepts an optional `version` param, and the summary already carries a `repaired` flag, but a reviewer re-reads a repaired story from scratch with no sense of what changed. Out of scope for the 2026-07-15 pass (the traversal-order/choice-label/coverage overhaul landed; the diff view is a separate, larger piece of work). | 2026-07-15 UX-improvements pass | Medium | Add a version selector using the existing param, and a passage-level diff (added/changed/removed nodes) between the current and previous version |
| U9 | No dark mode, no i18n readiness (all copy is inline JSX, no string catalog), and no push channel (WebSocket/SSE): every long-running status (generation, moderation review) is poll-only. All three were explicitly out of scope for the 2026-07-15 pass (product/design decisions or backend-architecture changes, not frontend polish). | 2026-07-15 UX-improvements pass | Low (dark mode, i18n) / Medium (push channel, as usage scales) | Dark mode: add a token set behind `prefers-color-scheme`. i18n: extract strings to a catalog while the surface is still small. Push channel: revisit if polling load becomes a real cost. |
| U10 | `docs/architecture/diagrams/journey-kid.puml`/`.svg` still show the old "Continue this story" label; the 2026-07-15 pass renamed it to "Ask for the next book" in the app but did not refresh the diagrams. | 2026-07-15 UX-improvements pass | Low | Regenerate the kid-journey diagram to match the current copy |
| U11 | The guardian responsive pass (2026-07-15) added a `~640px` breakpoint covering the review-queue action row, profile/book cards, and tables in `guardian.css`, plus a 44px floor on native form controls and buttons app-wide; it did not do a full page-by-page mobile audit (e.g. `ConsolePage.tsx`'s device-management cards, `IntakePage.tsx`'s two-column layouts at in-between widths). | 2026-07-15 UX-improvements pass | Low | Do a dedicated mobile-viewport pass per guardian page once real usage data shows which ones matter most |
