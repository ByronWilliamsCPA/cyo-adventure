...
/**
 * #CRITICAL: security: the two halves below are ONE group, run serially, on
 * purpose. They used to be two independent top-level `describe` blocks, which
 * meant the presence anchor could fail while the invisibility assertions it
 * anchors still ran and still went green. An anchor that can be skipped while
 * the thing it anchors keeps running is not an anchor; the header above says
 * in as many words that without the anchor the invisibility assertion "would
 * go silently green the day a staging database reset wiped the seeded rows".
 *
 * `mode: 'serial'` on this outer group is what makes that structural:
 * Playwright skips every remaining test in a serial group once one fails, and
 * that skip reaches NESTED describes too, including their `beforeAll` hooks.
 * Verified by reproduction against this repo's installed Playwright: with the
 * anchor failing, the nested group reported "1 did not run" and its
 * `beforeAll` never executed; with the anchor passing, both ran.
 *
 * Serial mode alone still leaves one hole, so it is not the only guard: a
 * `--grep`-filtered or otherwise re-ordered invocation could reach the
 * invisibility group without the anchor having run at all, and a group whose
 * earlier test was never selected is not "failed", so nothing would be
 * skipped. `presenceConfirmed` below closes that: the invisibility group's
 * own `beforeAll` refuses to proceed unless the anchor set it.
 * #VERIFY: keep BOTH. Removing either one restores a path on which the
 * child-safety invisibility assertions can pass against a corpus whose
 * presence was never established.
 */
test.describe('moderation QA corpus containment on staging', () => {
  test.describe.configure({ mode: 'serial' })

  /**
   * Set by the presence anchor's last statement, read by the invisibility
   * group's `beforeAll`. Module state rather than a Playwright construct
   * because it must hold across the two nested describes in this one file and
   * must be false, not absent, when the anchor never ran.
   */
  ...
  test.describe('moderation QA corpus is present on staging (admin view)', () => {
    ...
    test.beforeAll(async ({ browser }) => {
      ...
      // #CRITICAL: security: hoisted here, next to the `newPage` it fixes, so
      // the "still on the app origin" precondition is structural rather than
      // something each call site has to remember. `storageState` seeds an
      // origin's localStorage but never visits it, so this page starts on
      // `about:blank`, whose opaque origin makes `window.localStorage` throw
      // `SecurityError: Failed to read the 'localStorage' property from
      // 'Window': Access is denied for this document.` The
      // `signInAsStagingTestUser` call this file used to make was the only
      // thing that navigated this page; the `storageState` refactor removed it
      // and left the anchor throwing before any assertion could run,
      // so the carefully written "re-run scripts/seed_moderation_qa.py"
      // diagnostic below was unreachable and the responder saw a raw
      // SecurityError instead. Reproduced against this repo's installed
      // Playwright, with a navigate-first control that passes.
      // #VERIFY: the app-origin assertion at the top of the test below fails
      // loudly if this navigation is ever removed again.
      await gotoResilient(adminPage, '/admin')
    })
    ...
    test('the admin master library lists every seeded mqa book', async ({ baseURL }) => {
      if (baseURL === undefined) {
        throw new Error(
          'the staging config must set baseURL; without it the app-origin ' +
            'check below is vacuous and the SecurityError regression could ' +
            'return unseen'
        )
      }
      expect(
        adminPage.url(),
        'the anchor must be on the app origin before reading localStorage. An ' +
          'un-navigated page sits on about:blank, whose opaque origin makes ' +
          'window.localStorage throw SecurityError, which would make this ' +
          'anchor permanently red for a reason unrelated to the corpus.'
      ...
      // Last statement in the anchor, deliberately: reached only when every
      // assertion above passed, so it cannot be set by a partially-successful
      // run. The invisibility group's `beforeAll` refuses to proceed without
      // it. See the outer describe's #CRITICAL note for why serial mode alone
      // is not enough.
  ...
  test.describe('mqa books are invisible to a kid profile on staging', () => {
    ...
    test.beforeAll(async ({ browser }) => {
      // #CRITICAL: security: refuse to run the invisibility assertions unless
      // the presence anchor actually passed. Serial mode on the outer group
      // covers "the anchor ran and failed"; this covers "the anchor never ran
      // at all" (a `--grep` filter, a re-ordering, a future refactor that
      // splits the file), where nothing is failed so nothing is skipped.
      // Without both, `toHaveCount(0)` over an empty corpus is a green that
      // means nothing.
      // #VERIFY: this must throw, not skip. A skipped safety assertion still
      // reads as "not failing" in a dashboard; a thrown one names its cause.
      if (!presenceConfirmed) {
        throw new Error(
          'the moderation QA presence anchor did not pass, so the invisibility ' +
            'assertions below would be checking that an unknown (possibly empty) ' +
            'corpus is invisible, which is vacuously true. Fix the anchor first: ' +
            'if it reported missing mqa books, re-run scripts/seed_moderation_qa.py ' +
            'against staging.'
        )
      }
