...
``_personalizable_slot_ids_for_job`` in ``test_moderation_pipeline.py`` and
...
async def test_resume_with_no_skeleton_slug_threads_empty_frozenset(
    ...
    exactly what ``_personalizable_slot_ids_for_story`` would answer for a job
...
async def test_resume_sentinel_free_skeleton_threads_empty_frozenset(
    ...
    monkeypatch.setattr(pipeline_mod, "load_skeleton", lambda _path: {"nodes": []})
    monkeypatch.setattr(
        pipeline_mod, "load_contract_for", lambda _path, _skeleton: None
    )
...
async def test_resume_missing_metadata_band_falls_back_to_brief_band(
    ...
    ``_personalizable_slot_ids_for_job``, not by re-reading the (absent) raw
    metadata key the way ``_personalizable_slot_ids_for_story`` would.
    """
    concept = Concept(
    ...
    monkeypatch.setattr(
        pipeline_mod, "resolve_skeleton_path", _capturing_resolve_skeleton_path
    )
    monkeypatch.setattr(pipeline_mod, "load_skeleton", lambda _path: {"nodes": []})
    monkeypatch.setattr(
        pipeline_mod,
...
async def test_resume_uncomputable_contract_threads_none_and_fails_closed(
    ...
    monkeypatch.setattr(pipeline_mod, "load_skeleton", _raise_not_found)
